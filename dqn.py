...
"""
Fork of PureJaxRL's DQN implementation: https://github.com/luchris429/purejaxrl/blob/main/purejaxrl/dqn.py
...
def make_train(config):
    ...
    def train(rng):
        ...
        def eps_greedy_exploration(rng, q_vals, t):
            rng_a, rng_e = jax.random.split(rng, 2)  # a key for sampling random actions and one for picking
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(train_state, rng):
                ...
                def _loss_fn(params):
                    q_vals = network.apply(params, learn_batch.first.obs)  # (batch_size, num_actions)
            ...
            if config.get("WANDB_MODE", "disabled") == "online":
                ...
                def callback(metrics):
                    if metrics["timesteps"] % 100 == 0:
                        # wandb.log(metrics)
                        mlflow.log_metrics(metrics, step=int(metrics["timesteps"]))
...
def main():
    ...
    mlflow.set_experiment(
        config["ENV_NAME"] + "." + str(int(config["TOTAL_TIMESTEPS"] // 1e6)) + "M" + "." + config["WANDB_PROJECT"]
    )
    mlflow.start_run()
